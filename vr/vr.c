...
{
    dlhandle = dlopen("libthermalclient.so", RTLD_NOW | RTLD_LOCAL);
    if (dlhandle) {
        dlerror();
        ...
        ALOGE("unable to open %s", "libthermalclient.so");
        return -1;
    }
...
error_handle:
    ALOGE("Error opening functions from %s", "libthermalclient.so");
